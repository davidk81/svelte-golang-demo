...
func testPatientToManyPatientidPatientNotes(t *testing.T) {
	...
	queries.Assign(&b.fkPatientId, a.Patientid)
	queries.Assign(&c.fkPatientId, a.Patientid)
	...
	for _, v := range check {
		if queries.Equal(v.fkPatientId, b.fkPatientId) {
			bFound = true
		}
		if queries.Equal(v.fkPatientId, c.fkPatientId) {
			cFound = true
		}
	}
...
func testPatientToManyAddOpPatientidPatientNotes(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.Patientid, first.fkPatientId) {
			t.Error("foreign key was wrong value", a.Patientid, first.fkPatientId)
		}
		if !queries.Equal(a.Patientid, second.fkPatientId) {
			t.Error("foreign key was wrong value", a.Patientid, second.fkPatientId)
		}
...
func testPatientToManySetOpPatientidPatientNotes(t *testing.T) {
	var err error
	...
	err = a.SetPatientidPatientNotes(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetPatientidPatientNotes(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.fkPatientId) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.fkPatientId) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.Patientid, d.fkPatientId) {
		t.Error("foreign key was wrong value", a.Patientid, d.fkPatientId)
	}
	if !queries.Equal(a.Patientid, e.fkPatientId) {
		t.Error("foreign key was wrong value", a.Patientid, e.fkPatientId)
	}

	if b.R.Patientid != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.Patientid != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.Patientid != &a {
	...
	if a.R.PatientidPatientNotes[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.PatientidPatientNotes[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testPatientToManyRemoveOpPatientidPatientNotes(t *testing.T) {
	var err error
	...
	err = a.AddPatientidPatientNotes(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemovePatientidPatientNotes(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.PatientidPatientNotes) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.PatientidPatientNotes[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.PatientidPatientNotes[0] != &e {
