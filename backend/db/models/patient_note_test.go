...
func testPatientNoteToOnePatientUsingPatientid(t *testing.T) {
	...
	queries.Assign(&local.fkPatientId, foreign.Patientid)
...
func testPatientNoteToOneUserUsingUserid(t *testing.T) {
	...
	queries.Assign(&local.fkUserId, foreign.Userid)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
...
func testPatientNoteToOneSetOpPatientUsingPatientid(t *testing.T) {
	...
	for i, x := range []*Patient{&b, &c} {
		...
		if !queries.Equal(a.fkPatientId, x.Patientid) {
			t.Error("foreign key was wrong value", a.fkPatientId)
		}

		zero := reflect.Zero(reflect.TypeOf(a.fkPatientId))
		reflect.Indirect(reflect.ValueOf(&a.fkPatientId)).Set(zero)
		...
		if !queries.Equal(a.fkPatientId, x.Patientid) {
			t.Error("foreign key was wrong value", a.fkPatientId, x.Patientid)
		}
	}
}
...
func testPatientNoteToOneRemoveOpPatientUsingPatientid(t *testing.T) {
	...
	if !queries.IsValuerNil(a.fkPatientId) {
...
func testPatientNoteToOneSetOpUserUsingUserid(t *testing.T) {
	...
	for i, x := range []*User{&b, &c} {
		...
		if !queries.Equal(a.fkUserId, x.Userid) {
			t.Error("foreign key was wrong value", a.fkUserId)
		}

		zero := reflect.Zero(reflect.TypeOf(a.fkUserId))
		reflect.Indirect(reflect.ValueOf(&a.fkUserId)).Set(zero)
		...
		if !queries.Equal(a.fkUserId, x.Userid) {
			t.Error("foreign key was wrong value", a.fkUserId, x.Userid)
		}
	}
}
...
func testPatientNoteToOneRemoveOpUserUsingUserid(t *testing.T) {
	...
	if !queries.IsValuerNil(a.fkUserId) {
		t.Error("foreign key value should be nil")
	}
...
var (
	patientNoteDBTypes = map[string]string{`Noteid`: `character varying`, `fkPatientId`: `character varying`, `fkUserId`: `character varying`, `Note`: `character varying`, `Created`: `timestamp with time zone`}
	_                  = bytes.MinRead
)
