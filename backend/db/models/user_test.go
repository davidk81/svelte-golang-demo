...
func testUserToManyUseridPatientNotes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.fkUserId, a.Userid)
	queries.Assign(&c.fkUserId, a.Userid)
	...
	check, err := a.UseridPatientNotes().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if queries.Equal(v.fkUserId, b.fkUserId) {
			bFound = true
		}
		if queries.Equal(v.fkUserId, c.fkUserId) {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUseridPatientNotes(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UseridPatientNotes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UseridPatientNotes = nil
	if err = a.L.LoadUseridPatientNotes(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UseridPatientNotes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUseridPatientNotes(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUseridPatientNotes(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.Userid, first.fkUserId) {
			t.Error("foreign key was wrong value", a.Userid, first.fkUserId)
		}
		if !queries.Equal(a.Userid, second.fkUserId) {
			t.Error("foreign key was wrong value", a.Userid, second.fkUserId)
		}

		if first.R.Userid != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Userid != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.UseridPatientNotes[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UseridPatientNotes[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}
...
func testUserToManySetOpUseridPatientNotes(t *testing.T) {
	...
	if !queries.IsValuerNil(b.fkUserId) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.fkUserId) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.Userid, d.fkUserId) {
		t.Error("foreign key was wrong value", a.Userid, d.fkUserId)
	}
	if !queries.Equal(a.Userid, e.fkUserId) {
		t.Error("foreign key was wrong value", a.Userid, e.fkUserId)
	}
...
var (
	userDBTypes = map[string]string{`Userid`: `character varying`, `Name`: `character varying`, `Roles`: `character varying`, `Secret`: `character varying`, `Created`: `timestamp with time zone`}
	_           = bytes.MinRead
)
