...
type PatientNote struct {
	...
	fkPatientId null.String `boil:"patientid" json:"patientid,omitempty" toml:"patientid" yaml:"patientid,omitempty"`
	fkUserId    null.String `boil:"userid" json:"userid,omitempty" toml:"userid" yaml:"userid,omitempty"`
...
var PatientNoteWhere = struct {
	...
	fkPatientId whereHelpernull_String
	fkUserId    whereHelpernull_String
	Note        whereHelpernull_String
	Created     whereHelpernull_Time
}{
	Noteid:      whereHelperstring{field: "\"patient_note\".\"noteid\""},
	fkPatientId: whereHelpernull_String{field: "\"patient_note\".\"patientid\""},
	fkUserId:    whereHelpernull_String{field: "\"patient_note\".\"userid\""},
...
type patientNoteR struct {
	Patientid *Patient `boil:"Patientid" json:"Patientid" toml:"Patientid" yaml:"Patientid"`
	Userid    *User    `boil:"Userid" json:"Userid" toml:"Userid" yaml:"Userid"`
}
...
func (o *PatientNote) Patientid(mods ...qm.QueryMod) patientQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"patientid\" = ?", o.fkPatientId),
	}
...
func (patientNoteL) LoadPatientid(ctx context.Context, e boil.ContextExecutor, singular bool, maybePatientNote interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.fkPatientId) {
			args = append(args, object.fkPatientId)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.fkPatientId) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.fkPatientId) {
				args = append(args, obj.fkPatientId)
			}
	...
	query := NewQuery(
		qm.From(`patient`),
		qm.WhereIn(`patient.patientid in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.fkPatientId, foreign.Patientid) {
...
func (patientNoteL) LoadUserid(ctx context.Context, e boil.ContextExecutor, singular bool, maybePatientNote interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.fkUserId) {
			args = append(args, object.fkUserId)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.fkUserId) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.fkUserId) {
				args = append(args, obj.fkUserId)
			}
	...
	query := NewQuery(
		qm.From(`user`),
		qm.WhereIn(`user.userid in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.fkUserId, foreign.Userid) {
...
func (o *PatientNote) SetPatientid(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Patient) error {
	...
	queries.Assign(&o.fkPatientId, related.Patientid)
	if o.R == nil {
		o.R = &patientNoteR{
			Patientid: related,
		}
...
func (o *PatientNote) RemovePatientid(ctx context.Context, exec boil.ContextExecutor, related *Patient) error {
	...
	queries.SetScanner(&o.fkPatientId, nil)
	...
	for i, ri := range related.R.PatientidPatientNotes {
		if queries.Equal(o.fkPatientId, ri.fkPatientId) {
			continue
		}
...
func (o *PatientNote) SetUserid(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	queries.Assign(&o.fkUserId, related.Userid)
	if o.R == nil {
		o.R = &patientNoteR{
			Userid: related,
		}
...
func (o *PatientNote) RemoveUserid(ctx context.Context, exec boil.ContextExecutor, related *User) error {
	...
	queries.SetScanner(&o.fkUserId, nil)
	...
	for i, ri := range related.R.UseridPatientNotes {
		if queries.Equal(o.fkUserId, ri.fkUserId) {
			continue
		}
