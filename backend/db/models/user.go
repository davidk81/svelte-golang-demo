...
var UserRels = struct {
	UseridPatientNotes string
}{
	UseridPatientNotes: "UseridPatientNotes",
}

// userR is where relationships are stored.
type userR struct {
	UseridPatientNotes PatientNoteSlice `boil:"UseridPatientNotes" json:"UseridPatientNotes" toml:"UseridPatientNotes" yaml:"UseridPatientNotes"`
}
...
var (
	userAllColumns            = []string{"userid", "name", "roles", "secret", "created"}
	userColumnsWithoutDefault = []string{"userid", "name", "roles", "secret"}
	userColumnsWithDefault    = []string{"created"}
...
// UseridPatientNotes retrieves all the patient_note's PatientNotes with an executor via userid column.
func (o *User) UseridPatientNotes(mods ...qm.QueryMod) patientNoteQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"patient_note\".\"userid\"=?", o.Userid),
	)
...
// LoadUseridPatientNotes allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUseridPatientNotes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`patient_note.userid in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UseridPatientNotes = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &patientNoteR{}
			}
			foreign.R.Userid = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.Userid, foreign.fkUserId) {
				local.R.UseridPatientNotes = append(local.R.UseridPatientNotes, foreign)
				if foreign.R == nil {
					foreign.R = &patientNoteR{}
				}
				foreign.R.Userid = local
				break
			}
		}
	}
...
// AddUseridPatientNotes adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UseridPatientNotes.
// Sets related.R.Userid appropriately.
func (o *User) AddUseridPatientNotes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PatientNote) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.fkUserId, o.Userid)
			...
			values := []interface{}{o.Userid, rel.Noteid}
	...
	if o.R == nil {
		o.R = &userR{
			UseridPatientNotes: related,
		}
	} else {
		o.R.UseridPatientNotes = append(o.R.UseridPatientNotes, related...)
	}
...
func (o *User) SetUseridPatientNotes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PatientNote) error {
	...
	if o.R != nil {
		for _, rel := range o.R.UseridPatientNotes {
			queries.SetScanner(&rel.fkUserId, nil)
			if rel.R == nil {
				continue
			}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, userid string, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"user\" where \"userid\"=$1", sel,
	)
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, userid string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"user\" where \"userid\"=$1 limit 1)"
